...
taggedNgram <- function(words.df, n) {
    dat <- lapply(1:(nrow(words.df)-n+1), FUN =
             function(x){
               c(ngram = paste(words.df$word[x:(x+n-1)] %>%
                                str_trim(),
                              collapse=" "), 
                 line = words.df$line[x], 
...
transcripts.df <- read_csv(infile)[1:20,]
...
# save processed file as an .RData file
save(list=paste0('processed_' ,n, 'gram_', gsub('[/].*[/]|[.]csv|-', '_', infile, perl = T)), 
     file=paste0(outpath, '/', 'processed_' ,n, 'gram_', gsub('[/].*[/]|[.]csv|-', '_', infile, perl = T), '.RData'))
